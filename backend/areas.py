"""Known Pikud HaOref alert areas.

Complete list of 1,418 alert areas as used in the Pikud HaOref (Home Front Command)
missile/rocket alert system. Sourced from the official Pikud HaOref API data.
Sorted alphabetically in Hebrew.

Note: Some previously listed area names have been updated to match the exact strings
used by the Pikud HaOref API:
  - Ashdod sub-areas updated (e.g. 'אשדוד - א,ב,ד,ה' instead of 'אשדוד - א,ב')
  - Herzliya split into 'הרצליה - מערב' and 'הרצליה - מרכז וגליל ים'
  - Hadera split into four sub-areas
  - Haifa 'קריות' area is now 'חיפה - קריית חיים ושמואל'
  - Haifa 'כרמל ועיר תחתית' is now 'חיפה - כרמל, הדר ועיר תחתית'
  - Netanya split into 'נתניה - מזרח' and 'נתניה - מערב'
  - 'קריית גת' is now 'קריית גת, כרמי גת'
  - 'שדרות, איבים, ניר עם' is now 'שדרות, איבים' (ניר עם is a separate entry)
"""
